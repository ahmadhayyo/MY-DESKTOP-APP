...
BUILTIN_INTEGRATIONS: dict[str, dict[str, Any]] = {
    ...
    "email": {
        "name": "Email (SMTP)",
        "icon": "📧",
        "description": "إرسال رسائل بريد إلكتروني مع مرفقات",
        "description_en": "Send emails with attachments via SMTP",
        "category": "messaging",
        "config_keys": ["SMTP_HOST", "SMTP_USER", "SMTP_PASS"],
        "setup_url": "https://support.google.com/accounts/answer/185833",
        "setup_hint": "لـ Gmail: فعّل التحقق بخطوتين ثم أنشئ App Password",
        "tools": ["send_email"],
    },
    "notion": {
        "name": "Notion",
    ...
    "http_api": {
        "name": "HTTP / REST API",
        "icon": "📡",
        "description": "استدعاء أي واجهة API على الويب + بيانات حيّة (طقس، كريبتو) بلا مفاتيح",
        "description_en": "Call any REST API + live data (weather, crypto) with no keys",
        "category": "custom",
        "config_keys": [],
        "setup_url": "",
        "setup_hint": "جاهز فوراً بلا إعداد — http_request, get_weather, get_crypto_price",
        "tools": ["http_request", "get_weather", "get_crypto_price"],
    },
    "trello": {
        "name": "Trello",
        "icon": "📋",
