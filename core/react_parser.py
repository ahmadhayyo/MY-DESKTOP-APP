...
def build_react_tool_prompt(tools: list[BaseTool]) -> str:
    ...
    lines = [
        ...
        "══════════════════════════════════════",
        "1. Your response MUST contain exactly ONE 'Action:' line followed by ONE 'Action Input:' line.",
        "2. NEVER write 'تم التنفيذ' or 'نجح' or 'أديت هذه الخطوة' — that is hallucination.",
        "3. NEVER describe what you will do — just do it with a tool call.",
        "4. Action must be EXACTLY one of the tool names listed below (case-sensitive).",
        "5. Action Input must be valid JSON with the correct parameter names.",
        "6. Do NOT output multiple Actions in one turn. One Action per turn only.",
        "",
        "EXAMPLES (study these carefully):",
        "",
        "--- Example 1: Open a website ---",
        "Thought: I need to open the browser and navigate to albomati.com",
        "Action: browser_open",
        'Action Input: {"url": "https://albomati.com"}',
        "",
        "--- Example 2: Download a song by name ---",
        "Thought: The user wants to download a song by name. I should use download_audio_by_search.",
        "Action: download_audio_by_search",
        'Action Input: {"query": "بينك وبيني تامر حسني", "dest": "desktop:"}',
        "",
        "--- Example 3: Get links from current page ---",
        "Thought: I need to find download links on this page.",
        "Action: browser_get_links",
        'Action Input: {"filter_text": "تحميل", "max_links": 20}',
        "",
        "--- Example 4: Download file to desktop ---",
        "Thought: I found the direct download URL, now I will save it to the Desktop.",
        "Action: browser_download_to_desktop",
        'Action Input: {"url": "https://example.com/song.mp3", "filename": "song.mp3"}',
        "",
        "Available tools:",
        "=" * 60,
    ]
